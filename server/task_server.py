...
class TaskServer(threading.Thread):
    ...
    def update(self, data):
        ...
        self.link_db.add_links(data['whitelist'].split(','), BerkeleyBTreeLinkDB.BEST_PRIORITY, 0)  # TODO: parse whitelist?
    ...
    def get_links_package(self, crawler):
        ...
        if links:
            ...
            self.cache(package_id, crawler, links)
            self.package_id += 1
            return package
        else:
            return None
    ...
    def readd_links(self, links):
        self.link_db.add_links(links, BerkeleyBTreeLinkDB.BEST_PRIORITY, 0)
    ...
    def put_data(self, package_id, data):
        if package_id in self.package_cache:
            ...
            for entry in data:
                ...
                self.link_db.add_links(entry['links'], BerkeleyBTreeLinkDB.DEFAULT_PRIORITY, 0, domain=entry['url'])
